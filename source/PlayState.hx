...
class PlayState extends MusicBeatState
{
	...
	override public function create()
	{
		...
		if ((cpuControlled || practiceMode) && (Paths.formatToSongPath(SONG.song) == 'trirotation' || Paths.formatToSongPath(SONG.song) == 'decay')) {
			cpuControlled = false;
			practiceMode = false;
		...
		judgementCounter.setFormat(Paths.font("Bronx.otf"), 16, FlxColor.WHITE, FlxTextAlign.LEFT, FlxTextBorderStyle.OUTLINE, FlxColor.BLACK);
		...
		botplayTxt = new FlxText(400, timeBarBG.y + 55, FlxG.width - 800, "fuck you", 32);
		...
		songWatermark.setFormat(Paths.font("Bronx.otf"), 16, FlxColor.WHITE, RIGHT, FlxTextBorderStyle.OUTLINE, FlxColor.BLACK);
		...
		if (creditsText)
		{
			...
			creditsWatermark.setFormat(Paths.font("Bronx.otf"), 16, FlxColor.WHITE, RIGHT, FlxTextBorderStyle.OUTLINE, FlxColor.BLACK);
		...
		engineBar = new FlxText(0, FlxG.height - 30, 0, "PFE v" + MainMenuState.psychforeverVersion, 16);
		engineBar.setFormat(Paths.font("Bronx.otf"), 16, FlxColor.WHITE, RIGHT, FlxTextBorderStyle.OUTLINE, FlxColor.BLACK);
	...
	override public function update(elapsed:Float)
	{
		...
		{
			if (Paths.formatToSongPath(SONG.song) == 'trirotation' || Paths.formatToSongPath(SONG.song) == 'decay')
			health = 0;
			else
			openChartEditor();
		}
	...
	private function popUpScore(note:Note = null):Void
	{
		...
		if (daRating.name == 'sick') {
			if ((ClientPrefs.healthtype == 'Kade') && health < 2) health += 0.08 * healthGain;
		}
		if (daRating.name == 'good') {
			score = 200;
			if ((ClientPrefs.healthtype == 'Kade') && health < 2) health += 0.03 * healthGain;
		}
		...
		if (ClientPrefs.healthtype == 'Psych') health += 0.02 * healthGain;
	...
	function opponentNoteHit(note:Note):Void
	{
		...
		if (health >= 0.1) {
			if (dad.curCharacter == 'shriek' || dad.curCharacter == 'evilori')
			health -= 0.01;
		}
